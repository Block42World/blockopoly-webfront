		...
		for(x[d]=-1; x[d]<dims[d]; ) {
			...
			for(j=0; j<dims[v]; ++j)
			{
				for(i=0; i<dims[u]; ) 
				{
					if(mask[n]) 
					{
						var w, h;//Width and Height of the face, 
						//Compute width (calculate the biggest w can be)
						...
						var done = false;
						for(h=1; j+h<dims[v]; ++h) //for all the pixel in that line
						{
							for(k=0; k<w; ++k) 
							{
